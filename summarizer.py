...
class Summarizer:
    def __init__(self, openai_api_key=None, vectorstore=None):
        ...
        self.llm4 = ChatOpenAI(
            openai_api_key=openai_api_key, temperature=0, model='gpt-4')
        ...
        self.qa = ConversationalRetrievalChain.from_llm(self.llm4, self.vectorstore.as_retriever(
        ), get_chat_history=self.get_chat_history, return_source_documents=True, condense_question_llm=self.llm35)

    def init_vectorstore(self, openai_api_key):
        embeddings = OpenAIEmbeddings(openai_api_key=openai_api_key)
        return PineconeVectorStore(index_name='video-transcripts', embedding=embeddings)
    ...
    def append_vectorstore(self, transcript):
        ...
        metadatas = [{'start': math.floor(
            t['start']), 'video_id': t['video_id']} for t in transcript]
